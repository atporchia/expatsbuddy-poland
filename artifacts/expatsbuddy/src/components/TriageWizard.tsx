...
import { TRIAGE_QUESTIONS, runTriage, type TriageAnswers } from "@/lib/triageRules";
...
export function TriageWizard({
  ...
  if (!done) {
    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <h2 className="mt-2 text-lg font-semibold text-slate-900">{q.question}</h2>
        <div className="mt-5 flex gap-3">
          ...
          {step > 0 && (
            <button
              ...
              className="ml-auto rounded-lg px-3 py-2.5 text-sm text-slate-500 hover:text-slate-800"
            >
              ← Back
            </button>
          )}
  ...
  const paths = result.pathSlugs.map((slug) => pathIndex[slug]).filter(Boolean);
  const categories = result.categorySlugs.map((slug) => categoryIndex[slug]).filter(Boolean);
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {paths.length > 0 && (
          <div className="mt-4">
            <h3 className="text-sm font-semibold text-slate-700">Start with these explainer pages</h3>
        ...
        {categories.length > 0 && (
          <div className="mt-4">
            <h3 className="text-sm font-semibold text-slate-700">You may also want to browse</h3>
            <ul className="mt-2 flex flex-wrap gap-2">
              {categories.map((c) => (
                <li key={c.slug}>
                  <Link
                    href={routes.category(c.slug)}
                    className="inline-block rounded-full border border-slate-300 bg-white px-4 py-1.5 text-sm font-medium text-slate-700 transition hover:border-blue-400 hover:text-blue-800"
                  >
                    {c.title}
                  </Link>
                </li>
        ...
        <button
          ...
          className="mt-5 text-sm text-slate-500 underline underline-offset-2 hover:text-slate-800"
        >
          Start over
        </button>
      </div>
