...
export default function HomePage({ locale }: { locale: string }) {
  ...
  const [pathCounts, setPathCounts] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    getCategories().then(async (cats) => {
      ...
      await Promise.all(
        cats.map(async (c) => {
          const paths = await getPathsForCategory(c.id);
          counts[c.id] = paths.length;
        }),
      );
