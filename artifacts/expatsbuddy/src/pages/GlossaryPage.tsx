...
export default function GlossaryPage() {
  ...
  useEffect(() => {
    getGlossaryTerms().then((t) => { setTerms(t); setLoading(false); });
  }, []);

  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Glossary</h1>
      ...
      {loading ? (
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {terms.map((t) => (
            <li key={t.id}>
              <Link
                href={routes.glossaryTerm(t.slug)}
